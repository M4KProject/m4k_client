...
export const updateRoute = (changes: Partial<Route>) => (
    setRoute({ ...getRoute(), ...changes })
);

const propGetter = <K extends keyof Route>(prop: K) => (
    (): Route[K] => getRoute()[prop]
);
...
const propSetter = <K extends keyof Route>(prop: K) => (
    (value: Route[K]) => updateRoute({ [prop]: value })
);
